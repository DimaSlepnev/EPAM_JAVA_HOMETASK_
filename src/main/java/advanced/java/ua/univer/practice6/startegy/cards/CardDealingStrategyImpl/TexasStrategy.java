...
public class TexasStrategy implements CardDealingStrategy {
    private final int PLAYER_CARDS_AMOUNT = 2;
    private final int COMMUNITY_STACK_AMOUNT = 5;
    Map<String, List<Card>> stack;

    @Override
    public Map<String, List<Card>> dealStacks(Deck deck, int players) {
        stack = new HashMap<>();
        List<Card> comCards = new ArrayList<>();
        stack.put("Community", comCards);
        ...
        for(int i =0; i < COMMUNITY_STACK_AMOUNT; i++){
            stack.get("Community").add(deck.dealCard());
        }
